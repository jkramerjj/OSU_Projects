...
from displayClass import DisplayProcess
from psqlwindow import PsqlWindow
...
def main(stdscr):
    ...
    variablemenu = {'in':'1 - Log into Database (functioning)', 'out':'1 - Log out of Database (functioning)'}
    selectmenu = [
        ## Used to house the Structure Window, until program logic is finalized
        "3 - Display Tables (functioning)",
        " ",


        ## The Following Options are not functional.....
        "2 - Display Structure Home (not-functioning)",
        "4 - Display Table Column Headers (not-functioning)",
        "5 - Select a Table to Query All (not-functioning)",
        "6 - Find Table to Query (not-functioning)",
        "7 - PSQL(ui-mostly-functioning)",
        "q - Quit (functioning)"
    ]
    ...
    while arg != ord('q'):
        ...
        ## Displays Talble Data
        elif arg == ord('3'):

            masterWindow.clear()
            masterWindow.border(0)
            masterWindow.refresh()

            #Searching DB for a table list
            results = SearchDB(conn,22)
            list = results.getTableList()

            #Creates a new window
            newTableWindow = masterWindow.stdscr.derwin(screenH-2, screenW-2, 1, 1)
            displayProcecss = DisplayProcess()
            displayProcecss.displayInitialList(stdscr, newTableWindow, list, displayHeaderWithEscapeOption)

            #Creats a window Box and refreshes
            newTableWindow.box()
            newTableWindow.refresh()
            ...
            while x != 27:  #27 == esc in ASCII
                ...
                displayProcecss.processKeyboardArrowInputs(stdscr, x)

                if x == ord("\n"):# and row_num != 0:
                    #Copies over hightlighted text that used pressed "enter" on
                    displayProcecss.retval = displayProcecss.highlighted_text_string
                    stdscr.erase()
                    stdscr.border(0)
                    break

                newTableWindow.erase()
                stdscr.border(0)
                newTableWindow.border(0)

                ## USED TO REPRINT AFTER USER'S KEYBOARD INPUT
                displayProcecss.displayListAfterArrowSelection(stdscr, newTableWindow, list, displayHeaderWithEscapeOption)
            ...
            psql = PsqlWindow(parent=masterWindow.stdscr)
            psql.createPsqlPad()
            # here's the query string, or None if they cancelled
            query = psql.run()
            successwin.clear()
            if query is not None:
                results = SearchDB(conn, query)
                list = results.getArtistList()
                newTableWindow = masterWindow.stdscr.derwin(screenH-2, screenW-2, 1, 1)
                displayProcecss = DisplayProcess()
                displayProcecss.displayInitialList(stdscr, newTableWindow, list, " ")
        ...
        ## Lost as of now.. This section needs help!
        elif arg == ord('2'):
